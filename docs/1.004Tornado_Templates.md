...
### 4.5.1 父模板
1. extends:`{% extend *filename* %}`继承模板，在子模板中会把父模板的所有内容都继承到子模板中，减少大量重复代码
...
class TemplatesHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        self.render('subtemplate.html',haha = self.haha) #传入
```
subtemplate.html代码
```
{% extends ./base.html %} #继承父模板
...
函数和类导入：
    render(
        ...
        {% from util.mod_file import sum , upper %} # 导入自定义模块方法
        {{ sum(10,10)}}     # 自定义模块使用方法
```

## 4.7 un_methodes和ui_modules
```
ui_methods/ui_uimodules:
